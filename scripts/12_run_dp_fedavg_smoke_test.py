...
def main() -> None:
    ...
    for round_number in range(1, MAX_ROUNDS + 1):
        ...
        pd.DataFrame(client_history).to_csv(CLIENT_HISTORY_PATH, index=False)
        pd.DataFrame(round_history).to_csv(ROUND_HISTORY_PATH, index=False)
    ...
    best_epsilons = [row["epsilon"] for row in best_privacy_rows]
    ...
    print(
        f"Best-round epsilon range: "
        f"{min(best_epsilons):.4f} - {max(best_epsilons):.4f}"
    )
    ...
    rows = [{
        ...
        "minimum_client_epsilon": min(best_epsilons),
        "maximum_client_epsilon": max(best_epsilons),
        "mean_client_epsilon": float(np.mean(best_epsilons)),
        "delta": DELTA,
        "noise_multiplier": NOISE_MULTIPLIER,
        "max_grad_norm": MAX_GRAD_NORM,
