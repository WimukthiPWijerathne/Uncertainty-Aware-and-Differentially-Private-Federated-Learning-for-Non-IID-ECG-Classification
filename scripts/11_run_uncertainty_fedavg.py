"""
Run a manual uncertainty-aware FedAvg experiment across four simulated hospitals.

Workflow:
1. Initialize one global ECG1DCNN.
2. Send global parameters to every hospital.
3. Train each hospital locally.
4. Estimate client uncertainty with Monte Carlo dropout.
5. Aggregate updates using sample count and predictive uncertainty.
6. Evaluate the global model on the shared validation set.
7. Save histories/checkpoints and evaluate the best model on the shared test set.

This is a manual uncertainty-aware FedAvg implementation and does not require
Flower simulation.
"""
...
from src.federated.ua_client import (
    UncertaintyAwareHospitalClient,
)
...
RUN_NAME = (
    f"ua_fedavg_rounds_{MAX_ROUNDS}"
...
ROUND_HISTORY_PATH = (
    LOG_DIR
    / "ua_fedavg_round_history.csv"
)

CLIENT_HISTORY_PATH = (
    LOG_DIR
    / "ua_fedavg_client_history.csv"
)
...
def create_clients(
    ...
    device: torch.device,
) -> list[UncertaintyAwareHospitalClient]:
    """Create one uncertainty-aware client for each hospital."""
    clients: list[
        UncertaintyAwareHospitalClient
    ] = []
    ...
    for hospital_id in range(NUM_HOSPITALS):
        ...
        client = UncertaintyAwareHospitalClient(
...
def save_global_checkpoint(
    ...
    """Save a global uncertainty-aware FedAvg checkpoint."""
    torch.save(
        {
            "round": round_number,
...
def main() -> None:
    ...
    for round_number in range(
        ...
        (
            ...
            client_results=client_aggregation_results,
            epsilon=UNCERTAINTY_EPSILON,
        )

        total_samples = sum(
            sample_count
            for _, sample_count, _ in client_aggregation_results
        )
        ...
        if len(current_round_rows) != len(clients):
            raise RuntimeError(
                "Client-history row count mismatch."
            )
        ...
        for (
            ...
            row["uncertainty_raw_weight"] = float(
                raw_weight
            )
            row["final_aggregation_weight"] = float(
                final_weight
            )

            print(
                f"Hospital {row['hospital_id']} | "
                f"uncertainty={row['uncertainty']:.8f} | "
                f"FedAvg weight={sample_only_weight:.4f} | "
                f"UA weight={final_weight:.4f}"
            )

        weight_sum = sum(aggregation_weights)
        print(f"UA aggregation-weight sum: {weight_sum:.8f}")
    ...
    print("\n" + "=" * 70)
    print("Best uncertainty-aware FedAvg global test results")
    print("=" * 70)
    ...
    print("\nUncertainty-aware FedAvg experiment completed.")
    print(
        f"Best checkpoint: "
        f"{BEST_CHECKPOINT_PATH}"
    )
